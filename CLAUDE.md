...
4. **`finabot/graph/graph.py:build_graph`** — LangGraph `StateGraph(AgentState)` with nodes: `supervisor` (= `call_llm_node` from `agents/nodes.py`), `market_analyst`, `fundamental_analyst`, `news_analyst`, `researchers`, `hold_analysis_pipeline`, `tool`. The supervisor is the entry point. Routing in `_internal_make_route_supervisor` inspects the **last AIMessage's `tool_calls`**:
   ...
   All non-end nodes loop back to `supervisor`. **规则预路由（`graph/router.py` + `router` 节点）**：多 Agent 图从 `START → router` 进入（`single_agent=True` 对照组保持 `START → supervisor`）。`classify_intent(question)` 纯规则/正则、无网络：命中"股票标的（6 位代码 / 公司后缀 / 常见名锚点）+ 持有意图词"→ 直接短路进 `hold_analysis_pipeline`；命中"市场级关键词且未点名个股"→ 短路进 `market_analyst`；命中"辩论关键词"→ 同时写 `debate_mode=True`；**其余一律回落 LLM supervisor**（含合规边界问题——`refusal.classify_question` 非 safe 必须走 LLM 以触发合规提示）。短路省一次 supervisor LLM 往返。`router` 节点必须至少写一个声明通道（写 `debate_mode`），LangGraph 不允许节点返回空更新。The full multi空 debate (news → bull+bear → summary) is **folded into `hold_analysis_pipeline`** as a single supervisor hop — `bull_researcher` / `bear_researcher` / `summary_manager` are NOT separate supervisor-routed graph nodes anymore; they live inside the pipeline (`agents/hold_pipeline.py`) and are invoked directly there. `hold_analysis_pipeline` accepts a `debate_mode` flag (LLM 路由路径取 tool_call 参数，规则短路路径取 `state["debate_mode"]`) to surface the step-by-step 新闻/看涨/看跌/结论稿件.
