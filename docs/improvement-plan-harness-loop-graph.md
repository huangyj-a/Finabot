...
15. ✅ 事实门 + 失败注入 + 最高风险回显（部分）：`graders.check_fact_traceability`（数字回溯证据）+ `agents/failure.py`（失败注入）+ `include_bear`/`FINABOT_NO_BEAR` + summary_manager 最高风险清单强制回显（`_SUMMARY_MANAGER_PROMPT` 规则 + `risk_flags` 传入；`tests/test_summary_manager.py`）。**`data_agent`/`report_agent` 节点、辩论证据流三分类与冲突记录（见 3.7/3.8）待续**
...
17. ✅ 部分 loop 护栏：多工具多参数文本解析（`extract_tool_calls_from_content` 支持多 `<tool_call>` 块/多参数，`tests/test_tool_call_parser.py`）+ `recursion_limit`（`FINABOT_MAX_RECURSION`，core.py/harness 接线）。**LLM 熔断 + 总 deadline、运行级 LLM 调用预算、子代理维度 telemetry、`memory/runtime/traces/` 落盘待续**
