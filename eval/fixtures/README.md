...
已采样 **8 个 fixture**（真实数据）：

| task | 标的 | 市场 | 数据 |
|---|---|---|---|
| t001/t003/t005/t019 | 贵州茅台 600519 | a | 历史行情（腾讯源 250 行）+ 估值 + 列表 |
| t002/t007 | 中国平安 601318 | a | 同上 |
| t009 | 宁德时代 300750 | a | 同上 |
| t012 | 易方达蓝筹精选 005827 | fund | 开放式基金净值（23961 行） |

**尚未采样**（数据源被墙或为概念题）：

- t010/t018（指数消歧/交易日）：`index_zh_a_hist`（eastmoney kline API）直连被 RST、新浪/腾讯指数源超时，无法采样；两题本质是概念题，可无数据回答。
- t011（港股与汇率）：`stock_hk_spot_em`/`stock_hk_hist`（eastmoney）被墙，无法采样。
- t004/t006/t008/t013/t014/t015/t016/t017/t020：通用/概念题，无需冻结数据。

A 股历史行情经腾讯源 `stock_zh_a_hist_tx` 采样（列名 `date/open/close/...`），
工具层 `_internal_history_metrics` 用 `_pick_column` 兼容 `收盘`/`close`。
